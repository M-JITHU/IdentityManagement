...
abstract contract ERC735  {
    ...
    function getClaim(bytes32 _claimId)virtual public pure returns(uint256 claimType, uint256 scheme, address issuer, bytes memory signature, bytes memory data, string memory uri);
    function getClaimIdsByType(uint256 _claimType)virtual public pure returns(bytes32[] memory claimIds);
